...
class ProcSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.ams.org/journals/bproc/all_issues.html']
    header = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36'
    }
    def __init__(self):
        self.client = pymongo.MongoClient(host='localhost', port=27017)
        self.db = self.client['publisher_journal_db']
        self.MONGO_TABLE = 'ams_journal_urls_col'

    def parse(self, response):
        issue_list=re.findall('<a href="/journals/proc/.*?">(.*?)</a>', response.text)
        for issue in issue_list:
            url='https://www.ams.org/journals/proc/'+issue+'/home.html'
            year=issue.split('-')[0]
            vol=issue.split('-')[1]
            iss=issue.split('-')[2]
            yield scrapy.Request(
                ...
                meta={'year':year,'vol':vol,'iss':iss,'issue':issue}
            )
    def parse_issue(self,response):
        article_list=re.findall('<a href="(.*?)">                Abstract, references and article information        </a>',response.text)
        for article in article_list:
            url='https://www.ams.org/journals/proc/'+response.meta['issue']+'/'+article
            yield scrapy.Request(
                url=url,
